...
function modulesYouBuyHtml(rows, { limit = 6 } = {}) {
  ...
  const item = (r, extra = false) =>
    `<li${extra ? ' class="hm-simple-buy__extra" hidden' : ""}>
      <span class="hm-simple-buy__left"><span class="hm-simple-buy__qty">${r.qty}×</span> <span class="hm-simple-buy__id">${escapeAttr(r.id)}</span></span>
    ...
    if (simpleDock) {
      const dockPrice =
        sysTot && sysTot.pricedQty > 0
          ? `${formatMoney(sysTot.total)} ${escapeAttr(sysTot.currency)}`
          : "—";
