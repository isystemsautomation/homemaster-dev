...
function formatPriceCell(p) {
  ...
  const stock =
    p.available === false
      ...
      : "";
  return `<span class="hm-bom__price">${formatMoney(p.amount)} ${escapeAttr(p.currency || "")}${stock}</span>`;
}
...
/** @deprecated Advanced Result still uses this three-block layout. */
function equipmentTotalsHtml({ sysTot, panelTot, meta = null }) {
  const hmAmount =
    sysTot && sysTot.pricedQty > 0 ? formatMoney(sysTot.total) : null;
  const hm = hmAmount
    ? `<div class="hm-totals__hero">
          <p class="hm-totals__hero-label">HomeMaster modules</p>
          <p class="hm-totals__hero-price">${hmAmount} <span>${escapeAttr(sysTot.currency)}</span></p>
          <p class="hm-totals__tag">exact · from the shop</p>
        </div>`
    : `<div class="hm-totals__hero">
          <p class="hm-totals__hero-label">HomeMaster modules</p>
  ...
  const panelAmount =
    panelTot && panelTot.lineCount > 0 ? formatMoney(panelTot.total) : null;
  const panel = panelAmount
    ? `<div class="hm-totals__row hm-totals__row--indicative">
          ...
          <span class="hm-totals__value">~ ${panelAmount} <span>${escapeAttr(panelTot.currency)}</span></span>
...
function formatCompanionPrice(a) {
  ...
  const conf =
    p.confidence === "confirmed"
      ? ""
      : ` <span class="hm-price-est" title="Indicative estimate">est.</span>`;
  return `<span class="hm-bom__indicative">~ ${formatMoney(line)} ${cur}${conf}</span>`;
}
  ...
  function panelResultBlock(panel, opts = {}) {
    ...
    const subLine =
      typeof modSubtotal === "number" && modSubtotal > 0
        ? `${formatMoney(modSubtotal)} ${escapeAttr(modCurrency)}`
        : "—";
    const under =
      missingPrice > 0
