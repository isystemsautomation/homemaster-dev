...
function estimatePanel(inputs, rules, prices = EMPTY, opts = {}) {
  ...
  let companionsFinal = finalizeEnclosureCompanions(
    companions,
    enclosure,
    rules,
    assumptions,
  ...
  enclosure = fitEnclosureToPackedLayout(
    enclosure,
    {
      ...
      accessories: companionsFinal,
      topology,
      enclosure,
    },
    rules,
  );
  companionsFinal = finalizeEnclosureCompanions(
    companions,
    enclosure,
    rules,
    assumptions,
...
function fitEnclosureToPackedLayout(enclosure, panel, rules) {
  ...
  if (layout.cabinets <= (enclosure.cabinets || 1)) return enclosure;
...
function buildCommercialSummary(estimate, prices, fieldSrc = {}) {
  ...
  const sameCurrency =
    ...
    systemDesigned.currency === otherMaterialsEstimatedTotal.currency;
  ...
  const completeHardwareEstimatedTotal =
    sameCurrency && otherMaterialsRequiredUnpricedCount === 0
      ? {
          total:
            Math.round(
              (systemDesigned.total + otherMaterialsEstimatedTotal.total) *
                100,
            ) / 100,
          currency: systemDesigned.currency,
          homemaster: systemDesigned.total,
          homemasterOrderable: cart?.total ?? null,
...
function panelMaterialLines(panel, estimate) {
  ...
  return accSrc
    .filter((a) => a.id !== "din_enclosure" && a.id !== "blanking_plate")
    .map((a) => ({
      panel: panel.name || "Panel",
...
async function buildEstimateXlsx(estimate, opts = {}) {
  ...
  function sheetFromRows(name, header, rows, pushRow) {
    ...
    if (subRows.length) {
      ...
      const totalCol = header.length - 2;
      const row = Array(header.length).fill("");
      row[0] = "Subtotal";
      row[totalCol] = { f: `SUM(${colLetter(totalCol)}${start}:${colLetter(totalCol)}${end})` };
      aoa.push(row);
    }
...
function packEnclosureLayout(items, enclosure) {
  ...
  for (const r of packedRows) {
    ...
    if (sec.rows.length >= rowsPer) secIdx += 1;
  }
...
function simpleCompleteHardwareHtml(comm) {
  ...
  const bits = [];
  if (hm) bits.push(`${formatMoney(hm.total)} ${escapeAttr(hm.currency)} HomeMaster modules`);
