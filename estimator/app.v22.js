...
/** True when the estimate reflects user equipment demand (not an empty / HA-only shell). */
function estimateHasUserDemand(result, rules) {
  const skip = new Set([
    "shutters",
  ...
  for (const [k, v] of Object.entries(result?.demand || {})) {
    if (skip.has(k)) continue;
    if (Number(v) > 0) return true;
  }
  if ((result?.shutters || 0) > 0) return true;
  const specs = rules?.modules ?? EMPTY;
  for (const m of result?.modules || []) {
    if (!specs[m.id]?.master && (m.qty || 0) > 0) return true;
  }
  return false;
}
...
function estimatePanel(inputs, rules, prices = EMPTY, opts = {}) {
  ...
  let enclosure = computeEnclosure(topology.modules, companions, rules);
  enclosure = fitEnclosureToPackedLayout(
    enclosure,
    {
...
function normalize(inputs, rules, assumptions) {
  ...
  const out = {
    ...
    motion_output:
      String(systemCfg.motion_output || inputs.motion_output || "dry_contact").toLowerCase() ===
      "transistor"
        ? "transistor"
        : "dry_contact",
    alarm_ma: 0,
    leak_shutoff_valves: 0,
    meta: {
      stage: inputs.stage || "design",
      ha_server: systemCfg.ha_server || "needed",
      manual_control: systemCfg.manual_control !== false,
...
function applySystemsDemand(out, systems, rules, assumptions) {
  ...
  const leakZones = Math.max(0, Number(w.leak_zones) || 0);
  if (leakZones > 0) {
    addDemand(out.demand, "leak_or_pulse", leakZones);
    // Cold + hot shut-off — WLD relays reserved; not in relay_out_3a pool.
    out.leak_shutoff_valves = 2;
  } else {
...
function applyRoomDemand(out, room, raw = room) {
  ...
  if (motion > 0) {
    const ch =
      out.motion_output === "transistor" ? "presence_in" : "di_dry";
    addDemand(out.demand, ch, motion);
  }
...
function poolProvides(modId, rules, opts = {}) {
  const out = { ...effectiveProvides(modId, rules) };
  for (const ch of RESERVE_ONLY_CHANNELS) delete out[ch];
  if (opts.reserveWldRelays && modId === "WLD-521-R1") {
    delete out.relay_out_3a;
  }
  return out;
}
...
function allocateWithPool(demand, rules, assumptions, controllerSupply, opts = {}) {
  ...
  while (guard++ < 200) {
    ...
    const caps = poolProvides(modId, rules, {
      reserveWldRelays: opts.reserveWldRelays,
    });
 ...
 */
function splitDemandAcrossSegments(demand, segments, rules, poolOpts = {}) {
  const remaining = {};
  for (const [ch, n] of Object.entries(demand || {})) {
  ...
  for (const seg of segments) {
    ...
    for (const slave of seg.slaves) {
      ...
      const caps = poolProvides(slave.id, rules, poolOpts);
...
function allocate(demand, rules, assumptions, normalized = EMPTY, prices = EMPTY) {
  ...
  const poolOpts = {
    reserveWldRelays:
      (Number(demand.leak_or_pulse) || 0) > 0 ||
      (Number(normalized.leak_shutoff_valves) || 0) > 0,
  };
  ...
  const contactorPart = allocateWithPool(contactorDemand, rules, assumptions, {}, {
    recordReserve: false,
    reserveWldRelays: poolOpts.reserveWldRelays,
  });
  ...
  if (segments.length === 0) {
    ...
    const segDemands = splitDemandAcrossSegments(
      bareDemand,
      segments,
      rules,
      poolOpts,
...
function round1Up(n) {
  return Math.ceil(Number(n) * 10 - 1e-9) / 10;
}
...
function buildCompanions(
  ...
  if (ma > 0) {
    const { v12: rail } = almSensorRails(rules);
    const almQty = (modulesList || []).reduce(
      (s, l) => s + (l.id === "ALM-173-R1" ? l.qty || 0 : 0),
      0,
    );
    const budget = rail * Math.max(almQty, 1);
    if (ma > budget) {
      out.push({
        ...
        qty: 1,
        label:
          labels.psu_sensor_12v ||
          "External 12 V sensor PSU (ALM rail overflow)",
        requirement: true,
        din_units: dinMap.psu_sensor_12v ?? 2,
        sku: "",
        unit_price: null,
        rating: null,
        role: "sensor_psu",
        note: `${ma} mA sensor draw exceeds ${budget} mA ALM rail budget.`,
      });
    }
  }
 ...
 * @param {number} dinNeeded
 * @param {(rowWidth: number) => number} [packedRowsAt] — when set, skip layouts
 *   whose section×row slots cannot hold the zone-packed row count.
 */
function pickCoupledEnclosure(dinNeeded, packedRowsAt) {
  ...
  for (let sections = 1; sections <= ENCLOSURE_MAX_SECTIONS; sections++) {
    ...
    /** @type {ReturnType<typeof pickEnclosureLayout>[]} */
    const layouts = [];
    if (packedRowsAt) {
      for (const rowWidth of ENCLOSURE_ROW_WIDTHS) {
        for (let rows = 1; rows <= ENCLOSURE_MAX_ROWS; rows++) {
          const capacity = rows * rowWidth;
          if (capacity < needPer) continue;
          const rowSlots = sections * rows;
          if (packedRowsAt(rowWidth) > rowSlots) continue;
          const { width_mm, height_mm } = enclosureDimsMm(rowWidth, rows);
          layouts.push({
            row_width: rowWidth,
            rows,
            ...
            aspect:
              Math.max(width_mm, height_mm) / Math.min(width_mm, height_mm),
            spare: capacity - needPer,
          });
        }
      }
    } else {
      const one = pickEnclosureLayout(needPer);
      if (one) layouts.push(one);
    }
    for (const layout of layouts) {
      ...
      if (
        ...
        (cand.cabinets === best.cabinets &&
          packedRowsAt &&
          cand.row_width > best.row_width) ||
        (cand.cabinets === best.cabinets &&
          (!packedRowsAt || cand.row_width === best.row_width) &&
          packedRowsAt &&
          cand.rows < best.rows) ||
        (cand.cabinets === best.cabinets &&
          (!packedRowsAt || cand.row_width === best.row_width) &&
          (!packedRowsAt || cand.rows === best.rows) &&
...
/** Upgrade enclosure when zone-packed rows exceed rated section×row slots. */
function fitEnclosureToPackedLayout(enclosure, panel, rules) {
  if (
    !enclosure ||
  ...
  const queue = buildPlacementQueue(
    { ...panel, accessories: panel.accessories || panel.companions || [] },
    rules,
  );
  const rowCheck = (rw) => countPackedLayoutRows(queue, rw);
  const packedRows = rowCheck(enclosure.row_width || 24);
  const slots =
    Math.max(1, Number(enclosure.cabinets) || 1) *
    Math.max(1, Number(enclosure.rows) || 1);
  if (packedRows <= slots) return enclosure;

  const layout = pickCoupledEnclosure(enclosure.din_needed, rowCheck);
  if (!layout) return enclosure;

  const dinFree = round2(layout.capacity - enclosure.din_occupied);
  return {
    ...enclosure,
...
function computeEnclosure(modulesList, companions, rules) {
  ...
  const dinOccupied = round2(dinTotal);
  ...
  const dinReserve = 0;
  const reservePct = 0;
  const dinNeeded = Math.ceil(dinOccupied - 1e-9);
  const layout = pickCoupledEnclosure(dinNeeded);

  if (!layout) {
    const maxCap = ENCLOSURE_MAX_CAPACITY * ENCLOSURE_MAX_SECTIONS;
    const panelsNeeded = Math.max(2, Math.ceil(dinNeeded / maxCap));
    return {
      status: "overflow",
      din_modules: round2(dinModules),
      din_companions: round2(dinCompanions),
      din_total: dinOccupied,
      din_occupied: dinOccupied,
      din_reserve: dinReserve,
      reserve_pct: reservePct,
      din_needed: dinNeeded,
      din_free: null,
      // Aliases for the quote / console (same values as din_*).
      occupied: dinOccupied,
      reserve: dinReserve,
      free: null,
      cabinets: null,
      row_width: null,
      ...
      height_mm: null,
      overflow: true,
      overflow_message: `${dinNeeded} M needed, ${maxCap} M max — split into ${panelsNeeded} panels`,
      max_capacity: ENCLOSURE_MAX_CAPACITY,
      max_sections: ENCLOSURE_MAX_SECTIONS,
      breakdown,
    };
  ...
  const dinFree = round2(layout.capacity - dinOccupied);
...
function finalizeEnclosureCompanions(
  ...
  const dinFree =
    enclosure.din_free != null
      ? Number(enclosure.din_free)
      : Math.max(0, capacity - occupied);
  const blankQty = Math.max(0, Math.ceil(dinFree - 1e-9));
  if (blankQty > 0) {
    out.push({
      id: "blanking_plate",
      qty: blankQty,
  ...
  const note =
    `Occupied ${enclosure.din_occupied} M · free ${enclosure.din_free} M · ` +
    `capacity ${capacity} M` +
...
HM.estimateHasUserDemand = estimateHasUserDemand;
...
/** Map of ISO currency → numeric amount from schema.org offers. */
function collectOfferAmounts(offers) {
  const list = asArray(offers)
    .flatMap((o) => {
      if (o && typeof o === "object" && Array.isArray(o.offers)) return o.offers;
      return [o];
  ...
  for (const o of list) {
    ...
    if (!cur || !Number.isFinite(n)) continue;
    out[cur] = n;
  }
  return out;
}
...
function parseProductLdJson(html, opts = {}) {
  ...
  for (const script of scripts) {
    ...
    for (const item of asArray(data)) {
      ...
      const amounts = collectOfferAmounts(item.offers);
      return {
        sku: String(sku),
        price,
  ...
  while (from < html.length) {
    ...
    for (const item of asArray(data)) {
      ...
      return {
        ...
        currencyAmbiguous: picked.ambiguous,
        amounts: collectOfferAmounts(item.offers),
      };
    }
  }
  return null;
...
function toEstimatePriceMap(prices) {
  ...
  for (const [key, info] of entries) {
    ...
    out[sku] = {
      ...
      amounts: info.amounts && typeof info.amounts === "object" ? info.amounts : undefined,
      amountEur:
        Number.isFinite(info.amounts?.EUR)
          ? info.amounts.EUR
          : String(info.currency || "").toUpperCase() === "EUR"
            ? info.price
            : undefined,
    };
  }
...
// ===== partners.js =====
/**
 * Partner / installer directory from the Odoo Resellers page (/partners).
 * Same approach as prices: fetch HTML + DOMParser (no API).
 ...
 * @returns {string}
 */
function resolveShopOrigin() {
  if (typeof location !== "undefined" && location?.origin) {
    const host = String(location.hostname || "");
    if (/home-master\.eu$/i.test(host) && !/^config\./i.test(host)) {
      return location.origin;
    }
  }
  return "https://www.home-master.eu";
}
 ...
 * @param {string} [origin]
 */
function partnersUrl(path = LISTING_PATH, origin = resolveShopOrigin()) {
  try {
...
/**
 * Cloudflare email-protection decode (data-cfemail).
...
function partnersDecodeHtmlEntities(s) {
  const t = String(s || "");
  if (typeof DOMParser !== "undefined") {
    const doc = new DOMParser().parseFromString(t, "text/html");
    return partnersCleanText(doc.documentElement?.textContent || t);
  }
  return partnersCleanText(
    t
      .replace(/&amp;/g, "&")
      .replace(/&nbsp;/g, " ")
      .replace(/&#160;/g, " ")
...
/**
 * Normalize Odoo grade labels for display.
 * @param {string} raw
 * @returns {{ key: string, label: string, sort: number }}
 */
function normalizePartnerCategory(raw) {
  const t = partnersDecodeHtmlEntities(raw).toLowerCase();
  if (/install/.test(t)) {
    return { key: "installer", label: "Installer", sort: 0 };
  }
  if (/distribut|shop/.test(t)) {
    return { key: "distributor", label: "Distributor", sort: 1 };
  }
  const label = partnersDecodeHtmlEntities(raw) || "Partner";
  return { key: "other", label, sort: 2 };
}

/**
 * @param {string} streetHtmlOrText
 */
function partnersFormatAddress(streetHtmlOrText) {
  if (!streetHtmlOrText) return null;
  let text = String(streetHtmlOrText);
...
/**
 * Parse schema.org Organization / PostalAddress microdata from a partner detail page.
 * @param {string} html
 * @returns {object|null}
 */
function parsePartnerDetail(html) {
  if (!html) return null;
  if (typeof DOMParser !== "undefined") {
    return parsePartnerDetailDom(html);
  }
  return parsePartnerDetailFallback(html);
}
...
function parsePartnerDetailDom(html) {
  ...
  const street =
    ...
    "";
  const address = partnersFormatAddress(street);
  const country =
    partnersCleanText(
  ...
  if (h1) {
    ...
    gradeNearName = partnersCleanText(
      wrap?.querySelector(".text-muted span")?.textContent || "",
    );
  }
  const category = normalizePartnerCategory(gradeNearName);

  if (!name && !address) return null;
  let resolvedCountry = country;
  if (!resolvedCountry && address) {
    const parts = address.split(",").map((s) => s.trim()).filter(Boolean);
    const last = parts[parts.length - 1];
    if (last && /^[A-Za-z][A-Za-z .'-]{1,40}$/.test(last)) {
      resolvedCountry = last;
    }
  }
  return {
    name,
...
function parsePartnerDetailFallback(html) {
  ...
  const gradeM =
    ...
    ) || html.match(/<span>(Installers|Distributors[^<]*)<\/span>/i);
  const category = normalizePartnerCategory(gradeM ? gradeM[1] : "");
  if (!name && !address) return null;
  let resolvedCountry = country;
  if (!resolvedCountry && address) {
    const parts = address.split(",").map((s) => s.trim()).filter(Boolean);
...
/**
 * Parse /partners?country_all=1 listing: countries + partner cards.
 * @param {string} html
 * @returns {{ countries: {id:string,name:string,path:string}[], partners: object[] }|null}
 */
function parsePartnersListing(html) {
  if (!html) return null;
  if (typeof DOMParser !== "undefined") {
    return parsePartnersListingDom(html);
  }
  return parsePartnersListingFallback(html);
}

function parsePartnersListingDom(html) {
  const doc = new DOMParser().parseFromString(html, "text/html");
  /** @type {{id:string,name:string,path:string}[]} */
  const countries = [];
  const seen = new Set();
  for (const a of doc.querySelectorAll('a[href*="/partners/country/"]')) {
    const href = a.getAttribute("href") || "";
    const path = href.split("?")[0];
    const m = path.match(/\/partners\/country\/([^/]+)/i);
    if (!m) continue;
    const id = m[1];
  ...
  const walk = root.querySelectorAll("h2, .card.h-100");
  for (const el of walk) {
    if (el.tagName === "H2") {
      const span = el.querySelector("span");
      const label = partnersCleanText(span?.textContent || el.textContent || "");
      if (/install|distribut|shop/i.test(label)) currentGrade = label;
      continue;
    }
    const href =
      el.querySelector("a.text-decoration-none")?.getAttribute("href") ||
      el.querySelector('a[aria-label="Go to partner"]')?.getAttribute("href") ||
      "";
    if (!href || /\/partners\/(country|grade)\//i.test(href)) continue;
    if (!/\/partners\//i.test(href)) continue;
    const name = partnersCleanText(
      el.querySelector(".card-title")?.textContent ||
        el.querySelector("h2")?.textContent ||
        "",
    );
    if (!name) continue;
    const country = partnersCleanText(
      el.querySelector('[itemprop="addressCountry"]')?.textContent || "",
    );
    const cat = normalizePartnerCategory(currentGrade);
    const path = href.split("?")[0];
    partners.push({
      id: path.replace(/^.*\//, ""),
      name,
  ...
  if (!countries.length && !partners.length) return null;
  return { countries, partners };
}

function parsePartnersListingFallback(html) {
  /** @type {{id:string,name:string,path:string}[]} */
  const countries = [];
  const seen = new Set();
  const countryRe =
    /href="(\/partners\/country\/([^"?]+))"[^>]*>\s*([^<]+)/gi;
  let cm;
  ...
  while (pos < html.length) {
    ...
    const h2 = slice.match(
      /<h2 class="mt-3 mb-2 col-12 h5-fs">\s*<span>([^<]+)<\/span>\s*<\/h2>/i,
    );
    const card = slice.match(
      /<div class="col-md-4 col-xl-3 col-12 mb-4">\s*<div class="card h-100[\s\S]*?<\/a>\s*<\/div>\s*<\/div>/i,
    );
    if (!h2 && !card) break;
    const h2i = h2 ? pos + h2.index : Infinity;
    const cardi = card ? pos + card.index : Infinity;
    if (h2 && h2i <= cardi) {
      grade = partnersDecodeHtmlEntities(h2[1]);
      pos = h2i + h2[0].length;
      continue;
    }
    const block = card[0];
    const hrefM = block.match(/href="(\/partners\/[^"?]+)"/i);
    const titleM = block.match(/card-title[^>]*>([^<]+)/i);
    const countryM = block.match(/itemprop="addressCountry"[^>]*>([^<]+)/i);
    if (hrefM && titleM) {
      const path = hrefM[1];
      if (!/\/partners\/(country|grade)\//i.test(path)) {
        const cat = normalizePartnerCategory(grade);
        partners.push({
          id: path.replace(/^.*\//, ""),
          name: partnersDecodeHtmlEntities(titleM[1]),
          path,
          country: countryM ? partnersDecodeHtmlEntities(countryM[1]) : null,
          category: cat.label,
          categoryKey: cat.key,
          categorySort: cat.sort,
          address: null,
...
/**
 * Merge listing row with detail page fields.
 * @param {object} base
 * @param {object|null} detail
 */
function mergePartnerDetail(base, detail) {
  if (!detail) return { ...base };
  return {
    ...base,
    name: detail.name || base.name,
    category: detail.category || base.category,
    categoryKey: detail.categoryKey || base.categoryKey,
    categorySort:
      detail.categorySort != null ? detail.categorySort : base.categorySort,
 ...
 */
function partnersCountryIdForName(countries, countryName) {
  if (!countryName) return null;
  const want = partnersCleanText(countryName).toLowerCase();
  const hit = countries.find((c) => c.name.toLowerCase() === want);
  if (hit) return hit.id;
  const bySlug = countries.find((c) =>
    c.id.toLowerCase().startsWith(want.replace(/\s+/g, "-")),
  );
  return bySlug?.id || null;
}

/**
 * @param {{ countries: object[], partners: object[] }} directory
 * @param {string} countryId
 */
function partnersForCountry(directory, countryId) {
  if (!directory || !countryId) return [];
  const country = directory.countries.find((c) => c.id === countryId);
  const name = country?.name || "";
  const list = (directory.partners || []).filter((p) => {
    if (p.countryId && p.countryId === countryId) return true;
    if (
      name &&
      p.country &&
      partnersCleanText(p.country).toLowerCase() === name.toLowerCase()
    ) {
      return true;
    }
    return false;
  ...
  return [...list].sort((a, b) => {
    const sa = a.categorySort ?? 9;
    const sb = b.categorySort ?? 9;
    if (sa !== sb) return sa - sb;
    return String(a.name || "").localeCompare(String(b.name || ""));
  });
}

function partnersCacheGet() {
  try {
    if (typeof sessionStorage === "undefined") return null;
    const raw = sessionStorage.getItem(CACHE_KEY);
    if (!raw) return null;
    const data = JSON.parse(raw);
    if (!data?.countries?.length && !data?.partners?.length) return null;
    return data;
  } catch {
    return null;
  }
...
function partnersCacheSet(data) {
  try {
    if (typeof sessionStorage === "undefined") return;
...
/**
 * Load partner directory (listing + detail enrichment). Returns null on failure.
 * @param {{
 ...
 * }} [opts]
 * @returns {Promise<{countries: object[], partners: object[], partnersUrl: string}|null>}
 */
async function fetchPartnersDirectory(opts = {}) {
  if (!opts.bypassCache) {
    const cached = partnersCacheGet();
    if (cached) return cached;
  }
  const fetchImpl =
  ...
  const origin = opts.origin || resolveShopOrigin();
  const listingUrl = partnersUrl(LISTING_PATH, origin);
  try {
    const ctrl =
    ...
    const html = await res.text();
    const parsed = parsePartnersListing(html);
    if (!parsed || (!parsed.countries.length && !parsed.partners.length)) {
      return null;
    }

    let partners = parsed.partners.map((p) => ({
      ...p,
      countryId: partnersCountryIdForName(parsed.countries, p.country),
      url: partnersUrl(p.path, origin),
    }));

    const enrich = opts.enrichDetails !== false;
    if (enrich && partners.length) {
      const details = await Promise.all(
        partners.map(async (p) => {
          try {
            const dres = await fetchImpl(p.url, {
              credentials: "same-origin",
              signal: ctrl,
            });
            if (!dres.ok) return { id: p.id, detail: null };
            const dhtml = await dres.text();
            return { id: p.id, detail: parsePartnerDetail(dhtml) };
          } catch {
      ...
      partners = partners.map((p) => {
        const merged = mergePartnerDetail(p, byId.get(p.id) || null);
        return {
          ...merged,
          countryId:
            partnersCountryIdForName(parsed.countries, merged.country) || p.countryId,
          url: p.url,
        };
      });
    }
    ...
    const directory = {
      ...
      partners,
      partnersUrl: partnersUrl("/partners", origin),
    };
    partnersCacheSet(directory);
    return directory;
  } catch {
    return null;
  }
...
HM.resolveShopOrigin = resolveShopOrigin;
HM.partnersUrl = partnersUrl;
HM.decodeCfEmail = decodeCfEmail;
HM.normalizePartnerCategory = normalizePartnerCategory;
HM.parsePartnerDetail = parsePartnerDetail;
HM.parsePartnersListing = parsePartnersListing;
HM.mergePartnerDetail = mergePartnerDetail;
HM.partnersForCountry = partnersForCountry;
HM.fetchPartnersDirectory = fetchPartnersDirectory;
...
/**
 * Shop amount in a requested currency, or null when that offer is missing.
 * @param {{amount?: number, currency?: string, amountEur?: number, amounts?: Record<string, number>}} p
 * @param {string|null|undefined} preferCurrency
 */
function priceAmountInCurrency(p, preferCurrency) {
  if (!p) return null;
  const prefer = preferCurrency ? String(preferCurrency).toUpperCase() : "";
  if (!prefer) {
    if (!Number.isFinite(p.amount)) return null;
    return { amount: p.amount, currency: p.currency || "EUR" };
  }
  if (String(p.currency || "").toUpperCase() === prefer && Number.isFinite(p.amount)) {
    return { amount: p.amount, currency: prefer };
  }
  const mapped = p.amounts?.[prefer];
  if (Number.isFinite(mapped)) return { amount: mapped, currency: prefer };
  if (prefer === "EUR" && Number.isFinite(p.amountEur)) {
    return { amount: p.amountEur, currency: "EUR" };
  }
  return null;
}
 ...
 * @param {object} estimate
 * @param {Record<string, {amount: number, currency: string, currencyAmbiguous?: boolean, available?: boolean, amountEur?: number, amounts?: Record<string, number>}>} prices
 * @param {{ preferCurrency?: string|null }} [opts]
 ...
 */
function systemTotal(estimate, prices, opts = {}) {
  const prefer = opts.preferCurrency || null;
  let total = 0;
  let currency = null;
  let pricedQty = 0;
  let lineCount = 0;
  ...
  for (const line of lines) {
    ...
    const picked = priceAmountInCurrency(p, prefer);
    if (!picked) continue;
    const qty = line.qty || 1;
    total += picked.amount * qty;
    currency = picked.currency || currency || "EUR";
    pricedQty += qty;
    lineCount += 1;
    if (p.available === false) {
      unavailableQty += qty;
 ...
 */
function cartTotal(estimate, prices, opts = {}) {
  const prefer = opts.preferCurrency || null;
  let total = 0;
  let currency = null;
  let pricedQty = 0;
  ...
  for (const line of lines) {
    ...
    total += picked.amount * (line.qty || 1);
    currency = picked.currency || currency || "EUR";
...
function buildFieldSideItems(src = {}) {
  ...
  const leakShutoff = Math.max(0, Number(systems?.water?.leak_zones) || 0) > 0;
  if (leakShutoff) {
    push(
      "shutoff_valve_motorised",
      "Motorised shut-off valve — powered externally, WLD relay provides dry contact only",
      2,
    );
  } else {
...
function buildCommercialSummary(estimate, prices, fieldSrc = {}) {
  ...
  const systemDesigned = systemTotal(estimate, prices);
  ...
  const homemasterCartTotal = {
    total: cart && cart.pricedQty > 0 ? cart.total : 0,
    currency: cart?.currency || systemDesigned?.currency || "EUR",
    pricedQty: cart?.pricedQty || 0,
    lineCount: cart?.lineCount || 0,
    moduleCount,
  ...
  const completeCurrency =
    otherMaterialsEstimatedTotal?.currency ||
    estimate?.reference_prices_meta?.currency ||
    null;
  const systemForComplete =
    (completeCurrency &&
      systemTotal(estimate, prices, { preferCurrency: completeCurrency })) ||
    systemDesigned;
  const cartForComplete =
    completeCurrency &&
    cartTotal(estimate, prices, { preferCurrency: completeCurrency });

  const sameCurrency =
    systemForComplete &&
    otherMaterialsEstimatedTotal &&
    systemForComplete.currency === otherMaterialsEstimatedTotal.currency;
  ...
  const completeHardwareEstimatedTotal =
    sameCurrency && otherMaterialsRequiredUnpricedCount === 0
      ? {
          total:
            Math.round(
              (systemForComplete.total + otherMaterialsEstimatedTotal.total) *
                100,
            ) / 100,
          currency: systemForComplete.currency,
          homemaster: systemForComplete.total,
          homemasterOrderable:
            cartForComplete &&
            cartForComplete.currency === systemForComplete.currency
              ? cartForComplete.total
              : cart && cart.currency === systemForComplete.currency
                ? cart.total
                : null,
...
function homemasterStockLabel(hm) {
  ...
  if (hm.unavailableModuleCount <= 0 && hm.availableModuleCount > 0) {
    return `${hm.moduleCount} module${hm.moduleCount === 1 ? "" : "s"} · all in stock`;
  }
  if (hm.availableModuleCount <= 0) {
    return `${hm.moduleCount} module${hm.moduleCount === 1 ? "" : "s"} · currently unavailable`;
  }
...
HM.priceAmountInCurrency = priceAmountInCurrency;
 ...
 *
 * Three sheets: HomeMaster order · Panel materials · Panel spec.
 */
...
/** 0-based AOA index and 1-based Excel column for the Line total subtotal cell. */
function xlsxLineTotalSubtotal(header) {
  const idx = header.indexOf("Line total");
  if (idx < 0) return null;
  return { aoaIndex: idx, excelColumn: colLetter(idx + 1) };
}

function panelHmLines(panel) {
  return (panel.modules || []).map((m) => ({
    ...
    supplier: "HomeMaster shop",
  }));
}

function panelMaterialLines(panel, estimate) {
  const accSrc =
  ...
  return accSrc.map((a) => ({
      ...
      supplier: "Your supplier / installer",
      id: a.id,
    }));
}

function panelSpecLines(panel, estimate) {
  const enc = panel.enclosure || (estimate.panels?.length === 1 ? estimate.enclosure : null);
  if (!enc || enc.status === "blocked") return [];
  const title = panel.name || "Panel";
  const lines = [];
  if (enc.status === "overflow" || enc.overflow) {
    lines.push({
      panel: title,
      label: enc.overflow_message || `Overflow — ${enc.din_needed} M needed`,
    });
    return lines;
  }
  ...
  lines.push({
    panel: title,
    label: `Occupied ${enc.din_occupied ?? enc.occupied ?? "—"} M · free ${enc.din_free ?? enc.free ?? "—"} M · capacity ${enc.capacity ?? "—"} M`,
  });
  if (enc.blanking_qty > 0) {
    lines.push({
      panel: title,
      label: `${enc.blanking_qty}× DIN blanking plate`,
    });
  }
  return lines;
}
...
/**
 * Structured export rows — pure (no SheetJS). Used by buildEstimateXlsx and tests.
 *
 * @param {object} estimate
 * @returns {{ homemaster: object[], materials: object[], spec: object[] }}
 */
function estimateExportSections(estimate) {
  ...
  if (panels.length > 0) {
    for (const panel of panels) {
      homemaster.push(...panelHmLines(panel));
      materials.push(...panelMaterialLines(panel, estimate));
      spec.push(...panelSpecLines(panel, estimate));
    }
  } else {
    homemaster.push(
    ...
    spec.push(...panelSpecLines({ name: "Project", enclosure: estimate.enclosure }, estimate));
  }

  return { homemaster, materials, spec };
}
...
async function buildEstimateXlsx(estimate, opts = {}) {
  ...
  const { homemaster, materials, spec } = estimateExportSections(estimate);
  const wb = XLSX.utils.book_new();

  function sheetFromRows(name, header, rows, pushRow) {
    const aoa = [header];
    let r = 2;
    const subRows = [];
    for (const line of rows) {
      pushRow(aoa, line, r);
      if (line.qty != null && line.unit_price !== "" && Number.isFinite(Number(line.unit_price))) {
        subRows.push(r);
      }
      r += 1;
    }
    if (subRows.length) {
      const start = subRows[0];
      const end = subRows[subRows.length - 1];
      const totalIdx = header.indexOf("Line total");
      const col1 = totalIdx >= 0 ? totalIdx + 1 : header.length - 1;
      const row = Array(header.length).fill("");
      row[0] = "Subtotal";
      row[col1 - 1] = {
        f: `SUM(${colLetter(col1)}${start}:${colLetter(col1)}${end})`,
      };
      aoa.push(row);
    }
  ...
  sheetFromRows(
    "HomeMaster order",
    ["Panel", "SKU", "Module", "Qty", "Unit price", "Line total", "Supplier", "Availability"],
    homemaster,
    (aoa, line, r) => {
      const price = line.unit_price !== "" ? Number(line.unit_price) : "";
      const avail =
  ...
  sheetFromRows(
    "Panel materials",
    ["Panel", "Item", "Qty", "Unit price", "Line total", "Supplier", "Pricing"],
    materials,
    (aoa, line, r) => {
      const price = line.unit_price !== "" ? Number(line.unit_price) : "";
      const pricing = line.indicative
      ...
      aoa.push([
        ...
        price,
        { f: `C${r}*D${r}` },
        line.supplier || "Your supplier / installer",
        pricing,
      ]);
    },
  );
  ...
  const specAoa = [
    ["HomeMaster estimate — panel specification"],
    [`Currency (shop modules): ${currency}`],
    ["Spec rows are quantities / layout only — no shop price"],
    [],
    ["Panel", "Specification"],
  ];
  for (const line of spec) {
    specAoa.push([line.panel, line.label]);
  }
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(specAoa), "Panel spec");
...
function estimateToCsv(estimate) {
  const { homemaster, materials } = estimateExportSections(estimate);
  const rows = [["Sheet", "Panel", "Kind", "SKU", "Label", "Qty", "Unit price", "Supplier"]];
  for (const line of homemaster) {
    rows.push([
      "HomeMaster order",
      line.panel,
      "module",
...
HM.xlsxLineTotalSubtotal = xlsxLineTotalSubtotal;
...
function adaptPresetForAvailability(preset, isAvailable) {
  ...
  if (!avail("STR-3221-R1")) {
    // UFH needs STR; motion only when 3-wire transistor output is selected.
    if ((Number(p.totals?.ufh_loops) || 0) > 0) {
      p.totals.ufh_loops = 0;
    }
    if (
      (p.systems?.system?.motion_output || "dry_contact") === "transistor" &&
      (Number(p.totals?.motion_sensors) || 0) > 0
    ) {
 ...
 * Zone 1 — logic PSU left of first controller, HomeMaster modules
 *   (LED PS left of each RGB/STR), network gear;
 * Zone 2 — 24 V branch fuses, then protection (MCB / RCD / contactors);
 * Zone 3 — distribution & field terminals, N/PE bars.
 */
function buildPlacementQueue(panel, rules = {}) {
  ...
  // Zone 1 — logic PSU left of first controller, then modules (topology order).
  ...
  // Zone 2 — fuses then protection
  fromAcc("fuse_24v_branch", "protection");
  ...
  // Zone 3 — terminals & bars
  fromAcc("terminal_24v", "field");
...
const CONTROLLER_IDS = new Set(["MicroPLC", "MiniPLC"]);
const LOGIC_PSU_IDS = new Set(["psu_din_60w", "psu_din_100w"]);
const LED_STR = "psu_led_str";
const LED_RGB = "psu_led_rgb";

/**
 * Layout zone for a tile. Rows never mix zones.
 * 1 = modules + PSUs, 2 = protection/fuses, 3 = terminals/bars.
 */
function itemZone(item) {
  ...
  if (
    id.startsWith("terminal_") ||
    id.startsWith("n_bar") ||
    id.startsWith("pe_bar")
  ) {
    return 3;
  }
  const g = item?.group;
  if (g === "protection" || g === "incomer") return 2;
  if (g === "field") return 3;
  return 1;
}

function isLedModulePair(a, b) {
  return (
    (a.id === LED_STR && b.id === "STR-3221-R1") ||
    (a.id === LED_RGB && b.id === "RGB-621-R1") ||
    (a.id === "STR-3221-R1" && b.id === LED_STR) ||
    (a.id === "RGB-621-R1" && b.id === LED_RGB)
  );
}

function isLogicControllerPair(a, b) {
  return LOGIC_PSU_IDS.has(a.id) && CONTROLLER_IDS.has(b.id);
}

/**
 * Atomic placement blocks. PSU sits left of its module; pairs never split.
 * Widths come from the tiles (STR+LED ≈ 13 M, RGB+LED ≈ 7 M with current DIN).
 */
function placementChunks(items) {
  ...
  for (let i = 0; i < (items || []).length; i++) {
    const cur = items[i];
    const nxt = items[i + 1];
    if (nxt && (isLedModulePair(cur, nxt) || isLogicControllerPair(cur, nxt))) {
      // Normalise LED/module order to [PSU][module] when legacy order appears.
      let left = cur;
      let right = nxt;
      if (
        (cur.id === "STR-3221-R1" && nxt.id === LED_STR) ||
        (cur.id === "RGB-621-R1" && nxt.id === LED_RGB)
      ) {
        left = nxt;
        right = cur;
      }
      const u0 = Number(left.units) || 0;
      const u1 = Number(right.units) || 0;
      chunks.push({
        items: [left, right],
        units: u0 + u1,
        zone: itemZone(left),
...
function fillRowBlanking(rowItems, used, rowWidth, eps) {
  const remain = rowWidth - used;
  if (remain <= eps) return rowItems;
  let left = remain;
  while (left > 1 + eps) {
...
/**
 * Zone-aware First Fit row packing (modules → protection → terminals).
 * @returns {{ items: object[], used: number, zone: number }[]}
 */
function packRowsByZone(items, rowWidth) {
  const rw = Math.max(1, Number(rowWidth) || 24);
  const eps = 1e-6;
  const blocks = placementChunks(items).filter(
    (c) => (Number(c.units) || 0) > 0,
  );
  const byZone = { 1: [], 2: [], 3: [] };
  for (const b of blocks) {
    const z = b.zone === 2 || b.zone === 3 ? b.zone : 1;
    byZone[z].push(b);
  }

  /** @type {{ items: object[], used: number, zone: number }[]} */
  const packedRows = [];

  for (const zone of [1, 2, 3]) {
    const zoneBlocks = byZone[zone];
    if (!zoneBlocks.length) continue;
    /** @type {{ items: object[], used: number, zone: number }[]} */
    const zoneRows = [];

    for (const block of zoneBlocks) {
      const units = Number(block.units) || 0;
      let target = zoneRows.find((r) => r.used + units <= rw + eps);
      if (!target) {
        target = { items: [], used: 0, zone };
        zoneRows.push(target);
      }
      for (const item of block.items) {
        target.items.push(item);
        target.used += Number(item.units) || 0;
      }
    }

    for (const r of zoneRows) {
      fillRowBlanking(r.items, r.used, rw, eps);
      packedRows.push(r);
    }
  }
...
/** Packed row count for a given row width (zone rows never mix). */
function countPackedLayoutRows(items, rowWidth) {
  return packRowsByZone(items, rowWidth).length;
}

/**
 * Pack items into enclosure rows/sections.
 * Three zones (modules → protection → terminals) never share a row.
 * Inside a zone: First Fit over all open rows of that zone (not only the
 * last). Atomic blocks stay intact. Block order is preserved (no width sort).
 */
function packEnclosureLayout(items, enclosure) {
  ...
  function rowLabel(list) {
    ...
    const zone = itemZone(real[0]);
    if (zone === 2) return "Protection";
    if (zone === 3) return "Field terminals & bars";
    ...
    if (seg) {
      ...
      const ledNote = led.length ? ` + ${led.length} LED PS` : "";
  ...
  const packedRows = packRowsByZone(items, rowWidth);

  // Map packed rows into coupled enclosure sections (rowsPer per section).
  const sections = [];
  let secIdx = 0;
  ...
  for (const r of packedRows) {
    ...
    sec.rows.push({
      items: r.items,
      label: rowLabel(r.items),
      used: rowWidth,
      zone: r.zone,
    });
    if (sec.rows.length >= rowsPer && secIdx < cabinets - 1) secIdx += 1;
  }
  ...
  const ratedCapacity = cabinets * rowsPer * rowWidth;
  ...
  return {
    ...
    sections: usedSections.length ? usedSections : [{ index: 1, rows: [] }],
    summary: {
...
function renderPanelLayoutHtml(layout, manifest, opts = {}) {
  ...
  const cab = layout.summary.rated_cabinets ?? layout.cabinets;
  const ratedRows = layout.summary.rated_rows ?? layout.rows;
  const cap = layout.capacity;
  const layoutBits =
    cab > 1
      ? `${cab} × (${ratedRows} rows × ${layout.row_width}) = ${cap} modules`
      : `${ratedRows} rows × ${layout.row_width} = ${each ?? cap} modules`;
  const dinBits = [
...
HM.itemZone = itemZone;
HM.placementChunks = placementChunks;
HM.countPackedLayoutRows = countPackedLayoutRows;
...
function buildSharePayload(state) {
  ...
  if (!state.expert) {
    ...
    if (sys.motion_output === "transistor") g.t = 1;
...
function expandSharePayload(payload) {
  ...
  let systems = {
    ...
    system: { manual_control: true, ha_server: "needed", motion_output: "dry_contact" },
  };
  ...
  if (!expert && payload.y && typeof payload.y === "object") {
    ...
    if (payload.y.g) {
      ...
      if (g.t === 1) systems.system.motion_output = "transistor";
    }
  }
...
/** Session-only — do not persist Simple/Advanced across browser visits. */
const MODE_KEY = "hm_configurator_ui_mode";
const MODE_HINT_KEY = "hm_configurator_adv_hint_dismissed";

/** Core equipment rows always shown in Simple (including zeros). */
const SIMPLE_EQUIP_CORE = new Set([
  "lights_onoff",
  "lights_dimmable",
  "switches",
  "leak_sensors",
...
/** In-memory only — not persisted. undefined=not loaded, null=unavailable, object=ok. */
let partnersDirectory = undefined;
/** Selected country id for installer finder — session UI only, not saved. */
let installerCountryId = "";
...
const COMPANION_NOTE_BY_ID = {
  ...
  cable_rs485: "Internal RS-485 panel bus",
};
...
function loadState() {
  try {
    ...
    // Mode is session-scoped — saved Advanced must not stick across visits.
    try {
      localStorage.removeItem(MODE_KEY);
      const sessionMode = sessionStorage.getItem(MODE_KEY);
      state.ui_mode =
        sessionMode === "advanced" || sessionMode === "simple"
          ? sessionMode
          : "simple";
    } catch {
...
function buildInputs() {
  const resolvedPanels = resolvePanels({ panels: state.panels });
  const useSinglePanel =
    state.ui_mode === "simple" && resolvedPanels.length <= 1;
  const panels = useSinglePanel
    ? [{ id: "panel-1", name: "Main panel", location: "" }]
    : resolvedPanels;
  ...
  return {
    ...
    rooms: state.rooms.map((r) => ({
      ...r,
      panel_id:
        useSinglePanel ? mainId : r.panel_id || mainId,
    })),
    systems,
  };
}
...
function formatPrice(p) {
  ...
  const stock =
    p.available === false
      ...
      : "";
  return `${formatMoney(p.amount)} ${p.currency || ""}${amb}${stock}`;
}

function formatPriceCell(p, qty = 1) {
  if (!p || typeof p.amount !== "number" || !Number.isFinite(p.amount)) {
    return `<span class="hm-price-dot" title="Price unavailable — not counted in the total" aria-label="Price unavailable"></span>`;
  }
  const cur = escapeAttr(p.currency || "");
  const n = Number(qty) || 1;
  const line = Math.round(p.amount * n * 100) / 100;
  if (n > 1) {
    return `<span class="hm-bom__price">${formatMoney(p.amount)}${cur ? ` ${cur}` : ""} each · ${formatMoney(line)}${cur ? ` ${cur}` : ""}</span>`;
  }
  return `<span class="hm-bom__price">${formatMoney(line)}${cur ? ` ${cur}` : ""}</span>`;
}

function pricingBadge(kind) {
  const map = {
    shop: ["Shop", "hm-price-badge--shop"],
    indicative: ["Indicative", "hm-price-badge--indicative"],
    estimate: ["Estimate", "hm-price-badge--estimate"],
    unavailable: ["Unavailable", "hm-price-badge--oos"],
  };
  const [lab, cls] = map[kind] || map.estimate;
  return `<span class="hm-price-badge ${cls}">${lab}</span>`;
}

function lineWidthM(dinEach, qty) {
  if (dinEach == null || !Number.isFinite(Number(dinEach))) return "—";
  const line = Math.round(Number(dinEach) * (Number(qty) || 0) * 100) / 100;
  return `${line} M`;
}

/** Unavailable HomeMaster lines that still have a shop price (display only). */
function unavailablePricedModules(result, prices) {
  const mods = [
  ...
  for (const m of mods) {
    ...
    if (p && Number.isFinite(p.amount) && p.available === false) {
      out.push({ id: m.id || m.sku, sku: m.sku, qty: m.qty || 1, amount: p.amount, currency: p.currency });
    }
  }
  return out;
}

function materialsRequiredOptionalTotals(comm) {
  let req = 0;
  let opt = 0;
  let currency = null;
  let reqN = 0;
  let optN = 0;
  for (const cat of comm?.materialCategories || []) {
    for (const it of cat.items || []) {
      if (it.pricing !== "estimated" || it.lineTotal == null) continue;
      currency = it.currency || currency || "EUR";
  ...
  return {
    required:
      reqN > 0
        ? { total: Math.round(req * 100) / 100, currency: currency || "EUR" }
        : null,
    optional:
      optN > 0
        ? { total: Math.round(opt * 100) / 100, currency: currency || "EUR" }
        : null,
  };
}

function cartAddButtonLabel(tot) {
  if (!tot || tot.pricedQty <= 0) {
    return `${hmIcon("cart", 16)} Add to cart`;
  }
  const lines = tot.lineCount;
  const units = tot.pricedQty;
  return `${hmIcon("cart", 16)} Add ${lines} line${lines === 1 ? "" : "s"} · ${units} unit${units === 1 ? "" : "s"} to cart — ${formatMoney(tot.total)} ${escapeAttr(tot.currency)}`;
}
...
function collectResultNotes(result, prices, opts = {}) {
  ...
  if (!opts.omitShopUnavailable) {
    const unavailable = unavailablePricedModules(result, prices);
    if (unavailable.length) {
      const ids = unavailable.map((u) => u.id);
      const sys = systemTotal(result, prices);
      const cart = cartTotal(result, prices);
      const gap =
        sys && cart
          ? Math.round((sys.total - cart.total) * 100) / 100
          : unavailable.reduce((s, u) => s + u.amount * u.qty, 0);
      const cur = sys?.currency || cart?.currency || unavailable[0]?.currency || "EUR";
      notes.push({
        kind: "unavailable_money",
        title: `${unavailable.length} module${unavailable.length === 1 ? "" : "s"} unavailable — ${formatMoney(gap)} ${cur} of the price above cannot be ordered today`,
        detail: `Not available in the shop: ${ids.join(", ")} (${formatMoney(gap)} ${cur}).`,
      });
    }
  }
  return notes;
...
function resultNotesBarHtml(notes) {
  ...
  const money = notes.find((n) => n && typeof n === "object" && n.kind === "unavailable_money");
  const plain = notes
    .filter((n) => typeof n === "string")
    .map((n) => `<p class="hm-warn">${escapeAttr(n)}</p>`);
  if (money) {
    plain.unshift(`<p class="hm-warn">${escapeAttr(money.detail)}</p>`);
  }
  const title = money
    ? money.title
    : `${notes.length} note${notes.length === 1 ? "" : "s"} to review`;
  const open = !!money;
  return `<div class="hm-notes">
            <button type="button" class="hm-notes-bar" id="hm-notes-toggle" aria-expanded="${open ? "true" : "false"}">
              ${hmIcon("warning", 18)}
              <span>${escapeAttr(title)}</span>
              <span class="hm-notes-bar__chev">${hmIcon("chevron", 16)}</span>
            </button>
            <div class="hm-notes-body" id="hm-notes-body"${open ? "" : " hidden"}>
              ${plain.join("")}
            </div>
          </div>`;
}
...
/**
 * Shared Result hero — prices | actions+metrics | legal.
 * @param {{ priceHtml: string, legalHtml?: string, stats: Array<{icon:string,value:string|number,label:string,tone?:string}>, actionsHtml?: string }} opts
 */
function resultHeroHtml({ priceHtml, legalHtml = "", stats, actionsHtml = "" }) {
  ...
  const mid =
    actionsHtml || statsHtml
      ? `<div class="hm-result-hero__mid">${actions}<div class="hm-result-stats">${statsHtml}</div></div>`
      : "";
  const legal = legalHtml
    ? `<div class="hm-result-hero__legal">${legalHtml}</div>`
    : "";
...
/**
 * Simple header: HomeMaster modules only (checkout-eligible shop total).
 * @param {ReturnType<typeof buildCommercialSummary>} comm
 */
function simpleHomemasterPriceHtml(comm) {
  const hm = comm.homemasterCartTotal;
  if (!hm || !hm.pricedQty) {
    return `<div class="hm-hm-price">
      <h3 class="hm-hm-price__h">HomeMaster modules</h3>
      <p class="hm-hm-price__amt">—</p>
      <p class="hm-hm-price__tag">exact current shop price</p>
    </div>`;
  }
  return `<div class="hm-hm-price">
    <h3 class="hm-hm-price__h">HomeMaster modules</h3>
    <p class="hm-hm-price__amt">${formatMoney(hm.total)} <span>${escapeAttr(hm.currency)} incl. VAT</span></p>
    <p class="hm-hm-price__tag">exact current shop price</p>
  </div>`;
}

/**
 * Collapsed other-materials categories.
 * @param {ReturnType<typeof buildCommercialSummary>} comm
 */
function simpleOtherMaterialsHtml(comm) {
  const mats = comm.otherMaterialsEstimatedTotal;
  const hm = comm.homemasterCartTotal;
  const matsCurNote =
    mats &&
    hm &&
    mats.currency !== hm.currency
      ? ` · panel materials priced in ${escapeAttr(mats.currency)}`
      : "";
  const headAmt =
    mats && Number.isFinite(mats.total)
      ? `~ ${formatMoney(mats.total)} ${escapeAttr(mats.currency)}${matsCurNote}`
      : "price from your supplier";
  const cats = (comm.materialCategories || [])
    .map((cat) => {
      const catAmt =
        cat.total != null
          ? `~ ${formatMoney(cat.total)} ${escapeAttr(cat.currency)}`
          : "quantity calculated in the BOM, price from your supplier";
      const items = (cat.items || [])
        .map((it) => {
          const qty =
            it.qtyUnit === "m" && it.meters != null
              ? `${it.meters} m `
              : it.qty != null
                ? `${it.qty}× `
                : "";
          const price =
            it.pricing === "estimated" && it.lineTotal != null
              ? `~ ${formatMoney(it.lineTotal)} ${escapeAttr(it.currency)}`
              : "price from your supplier";
          return `<li><span class="hm-mat__item-lab">${qty}${escapeAttr(it.label)}</span><span class="hm-mat__item-amt">${price}</span></li>`;
        })
        .join("");
      return `<details class="hm-mat__cat">
        <summary class="hm-mat__cat-sum">
          <span class="hm-details-chev" aria-hidden="true">${hmIcon("chevron", 14)}</span>
          <span class="hm-mat__cat-lab">${escapeAttr(cat.label)}</span>
          <span class="hm-mat__cat-amt">${catAmt}</span>
        </summary>
        <ul class="hm-mat__items">${items}</ul>
      </details>`;
    })
    .join("");

  return `<details class="hm-card hm-mat hm-card--ref" open>
    <summary class="hm-mat__summary">
      <span class="hm-mat__summary-main">
        <span class="hm-details-chev" aria-hidden="true">${hmIcon("chevron", 16)}</span>
        <span class="hm-mat__title">Other required panel materials</span>
        <span class="hm-mat__badge">Not sold by HomeMaster · not added to cart</span>
      </span>
      <span class="hm-mat__total">${headAmt}</span>
    </summary>
    ${cats}
    <button type="button" class="hm-text-link hm-mat__expand-all" id="s-mat-expand">Show detailed panel BOM</button>
  </details>`;
}

/**
 * Complete hardware = modules + materials, only when formula is complete.
 * @param {ReturnType<typeof buildCommercialSummary>} comm
 */
function simpleCompleteHardwareHtml(comm) {
  const full = comm.completeHardwareEstimatedTotal;
  const hm = comm.homemasterCartTotal;
  const mats = comm.otherMaterialsEstimatedTotal;
  if (full) {
    const orderable =
      full.homemasterOrderable != null &&
      full.homemasterOrderable !== full.homemaster
        ? `<p class="hm-complete__orderable">Orderable today: ${formatMoney(full.homemasterOrderable)} ${escapeAttr(full.currency)}</p>`
        : "";
    return `<div class="hm-card hm-complete hm-card--ref">
      <h3 class="hm-complete__h">Complete hardware (as designed)</h3>
      <p class="hm-complete__amt">~ ${formatMoney(full.total)} <span>${escapeAttr(full.currency)}</span></p>
      <p class="hm-complete__formula">= ${formatMoney(full.homemaster)} HomeMaster modules + ~ ${formatMoney(full.materials)} panel materials</p>
      ${orderable}
      <p class="hm-complete__note">Installation not included · field side quoted by your installer.</p>
    </div>`;
  }
  const bits = [];
  if (hm && hm.pricedQty > 0) bits.push(`${formatMoney(hm.total)} ${escapeAttr(hm.currency)} HomeMaster modules`);
  if (mats) bits.push(`~ ${formatMoney(mats.total)} ${escapeAttr(mats.currency)} panel materials`);
  const formula = bits.length ? bits.join(" + ") : "breakdown incomplete";
  const reason =
    comm.otherMaterialsRequiredUnpricedCount > 0
      ? "Some required panel materials have no reliable price yet — complete total not shown."
      : hm && mats && hm.currency !== mats.currency
        ? "HomeMaster and panel materials are in different currencies — complete total not shown."
        : "Complete panel total is not available for this configuration.";
  return `<div class="hm-card hm-complete hm-complete--incomplete hm-card--ref">
    <h3 class="hm-complete__h">Complete hardware for this panel</h3>
    <p class="hm-complete__amt hm-muted">${escapeAttr(reason.includes("currencies") ? "Different currencies" : "Not available")}</p>
    <p class="hm-complete__formula">${formula}</p>
    <p class="hm-complete__note">${escapeAttr(reason)}</p>
  </div>`;
}

/**
 * Field-side equipment — quantities only, no prices.
 * @param {ReturnType<typeof buildCommercialSummary>} comm
 */
function simpleFieldSideBodyHtml(comm) {
  const items = comm.fieldSideItems || [];
  if (!items.length) return "";
  const rows = items
    .map((it) => {
      const qty = it.qty != null ? `${it.qty}× ` : "";
      return `<li><span class="hm-field__lab">${qty}${escapeAttr(it.label)}</span><span class="hm-field__note">${escapeAttr(it.note)}</span></li>`;
    })
    .join("");
  return `<div class="hm-field hm-field--nested">
    <h4 class="hm-field__h">Field side</h4>
    <ul class="hm-field__list">${rows}</ul>
  </div>`;
}

function simpleInstallationBodyHtml(comm) {
  const labels = comm.installationLabels || [];
  const list =
    labels.length > 0
      ? labels.map((l) => escapeAttr(l)).join(", ")
      : "panel assembly and internal wiring, field installation and cable laying, commissioning and handover";
  return `<div class="hm-installation hm-installation--nested">
    <h4 class="hm-installation__h">Installation — not included</h4>
    <p class="hm-installation__lead">Your installer quotes: ${list}.</p>
  </div>`;
}

/** Collapsed Field side + Installation — one quieter caveat block. */
function simpleInstallerQuotesHtml(comm) {
  const field = simpleFieldSideBodyHtml(comm);
  const install = simpleInstallationBodyHtml(comm);
  if (!field && !install) return "";
  return `<details class="hm-caveat-fold">
    <summary class="hm-caveat-fold__sum">
      <span class="hm-details-chev" aria-hidden="true">${hmIcon("chevron", 16)}</span>
      <span class="hm-caveat-fold__title">What your installer quotes</span>
    </summary>
    <div class="hm-caveat-fold__body">${field}${install}</div>
  </details>`;
}

/**
 * FIND AN INSTALLER — from /partners. Hidden entirely when directory is unavailable.
 * @param {{countries: object[], partners: object[], partnersUrl: string}|null|undefined} directory
 * @param {string} countryId
 */
function simpleInstallerHtml(directory, countryId) {
  if (!directory || !directory.countries?.length) return "";
  const allUrl = escapeAttr(directory.partnersUrl || "https://www.home-master.eu/partners");
  const opts = [
      ...
      if (installers.length) {
        ...
        if (distributors.length) {
          ...
          body += `<details class="hm-installer__dist">
            <summary class="hm-installer__dist-sum"><span class="hm-details-chev" aria-hidden="true">${hmIcon("chevron", 14)}</span> Show distributors (${n})</summary>
  ...
  return `<div class="hm-installer hm-installer--quiet" id="s-installer">
    <h3 class="hm-installer__h">Find an installer</h3>
    <label class="hm-installer__label" for="s-installer-country">Country</label>
    <select id="s-installer-country" class="hm-installer__select">${opts}</select>
    ${body}
  </div>`;
}

async function ensurePartnersDirectory() {
  if (partnersDirectory !== undefined) return partnersDirectory;
  try {
    partnersDirectory = await fetchPartnersDirectory();
  } catch {
    partnersDirectory = null;
  }
  return partnersDirectory;
}

/** Advanced Result totals — display only; uses cartTotal for purchasable amount. */
function equipmentTotalsHtml({ cartTot, sysTot, panelTot, comm, meta = null }) {
  const cur = (t) => escapeAttr(t?.currency || "EUR");
  const avail =
    cartTot && cartTot.pricedQty > 0
      ? `<div class="hm-totals__hero">
          <p class="hm-totals__hero-label">Available now</p>
          <p class="hm-totals__hero-price">${formatMoney(cartTot.total)} <span>${cur(cartTot)}</span></p>
          <p class="hm-totals__meta">${cartTot.lineCount} line${cartTot.lineCount === 1 ? "" : "s"} · ${cartTot.pricedQty} unit${cartTot.pricedQty === 1 ? "" : "s"}</p>
          <p class="hm-totals__badge">exact · from the shop</p>
        </div>`
      : `<div class="hm-totals__hero">
          <p class="hm-totals__hero-label">Available now</p>
          <p class="hm-totals__hero-price">—</p>
          <p class="hm-totals__badge">exact · from the shop</p>
        </div>`;

  let unavailHtml = "";
  if (sysTot && cartTot && sysTot.unavailableQty > 0) {
    const gap = Math.round((sysTot.total - cartTot.total) * 100) / 100;
    const names = (comm?.moduleRows || [])
      .filter((r) => r.status === "oos")
      .map((r) => r.id)
      .filter(Boolean);
    const nameBit = names.length ? names.join(", ") : `${sysTot.unavailableLineCount} line(s)`;
    unavailHtml = `<div class="hm-totals__row hm-totals__row--muted">
          <span class="hm-totals__label">Currently unavailable</span>
          <span class="hm-totals__value">${formatMoney(gap)} <span>${cur(sysTot)}</span></span>
          <span class="hm-totals__tag">${escapeAttr(nameBit)}</span>
        </div>`;
  }

  const split = materialsRequiredOptionalTotals(comm);
  const mats = comm?.otherMaterialsEstimatedTotal || panelTot;
  const panelHead = mats
    ? `<div class="hm-totals__row hm-totals__row--l2">
          <span class="hm-totals__label">Panel materials — you or your installer buy these</span>
          <span class="hm-totals__value">~ ${formatMoney(mats.total)} <span>${cur(mats)}</span></span>
          <span class="hm-totals__badge">not sold by HomeMaster · not in cart</span>
        </div>`
    : `<div class="hm-totals__row hm-totals__row--l2">
          <span class="hm-totals__label">Panel materials — you or your installer buy these</span>
  ...
  const reqRow =
    split.required
      ? `<div class="hm-totals__row hm-totals__row--indent">
          <span class="hm-totals__label">Required</span>
          <span class="hm-totals__value">~ ${formatMoney(split.required.total)} <span>${cur(split.required)}</span></span>
        </div>`
      : "";
  const optRow =
    split.optional
      ? `<div class="hm-totals__row hm-totals__row--indent">
          <span class="hm-totals__label">Optional (network &amp; server)</span>
          <span class="hm-totals__value">~ ${formatMoney(split.optional.total)} <span>${cur(split.optional)}</span></span>
        </div>`
      : "";
  ...
  const orderableNote =
    ...
    full.homemasterOrderable !== full.homemaster
      ? `<div class="hm-totals__row hm-totals__row--indent">
          <span class="hm-totals__label">Orderable today</span>
          <span class="hm-totals__value">${formatMoney(full.homemasterOrderable)} <span>${escapeAttr(full.currency)}</span></span>
        </div>`
      : "";
  const completeRow = full
    ? `<div class="hm-totals__row hm-totals__row--l2">
          <span class="hm-totals__label">Complete hardware (as designed)</span>
          <span class="hm-totals__value">~ ${formatMoney(full.total)} <span>${escapeAttr(full.currency)}</span></span>
          <span class="hm-totals__tag">= ${formatMoney(full.homemaster)} HomeMaster modules + ~ ${formatMoney(full.materials)} panel materials</span>
        </div>${orderableNote}`
    : "";
  ...
  return {
    ...
    legal: (() => {
      ...
      return `<p class="hm-totals__disclaimer">${escapeAttr(disclaimer)}</p>
...
function formatCompanionPrice(a) {
  const p = a?.price;
  const line = p?.line_total ?? p?.amount;
  if (!p || !Number.isFinite(line)) {
    return `<span class="hm-muted">—</span>`;
  }
  const cur = escapeAttr(p.currency || "EUR");
  return `<span class="hm-bom__indicative">~ ${formatMoney(line)} ${cur}</span>`;
}

function companionPricingKind(a) {
  const p = a?.price;
  if (!p || !Number.isFinite(p.line_total ?? p.amount)) return null;
  return p.confidence === "confirmed" ? "indicative" : "estimate";
}
...
function moduleBlurb(id) {
  const b = rules?.modules?.[id]?.blurb;
...
function formatModuleBuyPrice(row) {
  ...
  if (row.amount != null && Number.isFinite(row.amount)) {
    ...
    const unit = `${formatMoney(row.amount)}${cur ? ` ${cur}` : ""}`;
    const total = `${formatMoney(line)}${cur ? ` ${cur}` : ""}`;
    const priceBit =
      qty > 1
...
function modulesYouBuyHtml(rows, { limit = 6 } = {}) {
  ...
  const item = (r, extra = false) => {
    const blurb = moduleBlurb(r.id);
    const blurbHtml = blurb
      ? `<span class="hm-simple-buy__blurb">${escapeAttr(blurb)}</span>`
      : "";
    return `<li${extra ? ' class="hm-simple-buy__extra" hidden' : ""}>
      <span class="hm-simple-buy__left">
        <span class="hm-simple-buy__qty">${r.qty}×</span>
        <span class="hm-simple-buy__meta">
          <span class="hm-simple-buy__id">${escapeAttr(r.id)}</span>
          ${blurbHtml}
        </span>
      </span>
      <span class="hm-simple-buy__right">${formatModuleBuyPrice(r)}</span>
    </li>`;
  };
...
/** Status under the primary Add to cart button. */
function simpleCartStatusHtml(cartTot, sysTot) {
  const units = cartTot?.pricedQty || 0;
  const unavail = sysTot?.unavailableLineCount || 0;
  const bits = [`${units} unit${units === 1 ? "" : "s"}`];
  if (unavail > 0) {
    bits.push(`${unavail} module${unavail === 1 ? "" : "s"} unavailable`);
  }
  return `<p class="hm-simple-cta__status">${bits.join(" · ")}</p>`;
}

/** @deprecated kept for Advanced if referenced — Simple uses simpleCartStatusHtml */
function cartHintHtml(cartTot, sysTot) {
  ...
  if (!cartTot || !cartTot.pricedQty) {
    ...
    let text = `In cart now: ${formatMoney(cartTot.total)} ${escapeAttr(cartTot.currency)}`;
...
function channelUsageTable(usage, { fold = true, open = false } = {}) {
  ...
  return `<details class="hm-fold"${open ? " open" : ""}>
...
function notIncludedBlock(items) {
  ...
  return `<div class="hm-card hm-not-included">
    <h3 class="hm-not-included__h">Not included — to be specified by the electrical design</h3>
...
function installationBlock(items) {
  ...
  if (!list.length) return "";
  const body = list
    .map((row) => `<li>${escapeAttr(row.label || row.name_en)}</li>`)
    .join("");
  return `<div class="hm-card hm-installation">
    <h3 class="hm-installation__h">Installation</h3>
    <p class="hm-installation__lead">Quoted by your installer — leave these for their quote.</p>
...
function enclosureMessage(enclosure, { compact = false } = {}) {
  ...
  if (enclosure.status === "overflow" || enclosure.overflow) {
    const msg =
      enclosure.overflow_message ||
      `${enclosure.din_needed} M needed exceeds max ${(enclosure.max_sections || 4) * (enclosure.max_capacity || 144)} M — add another panel on screen 1 or reduce demand.`;
    return `<p class="hm-warn">Enclosure: ${escapeAttr(msg)}</p>`;
  }
  const occupied = enclosure.occupied ?? enclosure.din_occupied ?? enclosure.din_total;
  const free = enclosure.free ?? enclosure.din_free;
  const breakdown =
    occupied != null && free != null && enclosure.capacity != null
      ? `occupied ${occupied} M · free ${free} M · capacity ${enclosure.capacity} M`
    ...
    .map((n) => {
      const text =
        typeof n === "object" && n
          ? n.detail || n.title || ""
          : String(n ?? "");
      return `<p class="hm-warn">${escapeAttr(text)}</p>`;
    })
    .join("");
}
...
function mountConfigurator(root) {
  ...
  const shell = el(`
    <div class="hm-estimator">
      <header class="hm-estimator__header">
        <p class="hm-estimator__tagline">Describe your home — get the module list, the panel layout and the price.</p>
        <div class="hm-estimator__mode-row">
        ...
        <p class="hm-mode-caption" id="hm-mode-caption"></p>
        <p class="hm-mode-hint" id="hm-mode-hint" hidden>
          You're in Advanced. Switch to Simple for the short version.
          <button type="button" class="hm-mode-hint__dismiss" id="hm-mode-hint-dismiss">Got it</button>
        </p>
      </header>
  ...
  const modeCaption = shell.querySelector("#hm-mode-caption");
  const modeHint = shell.querySelector("#hm-mode-hint");
  const modeHintDismiss = shell.querySelector("#hm-mode-hint-dismiss");
  const estimatorRoot = shell;

  function setUiMode(mode, { syncFromRooms = false, step, scrollTo } = {}) {
  ...
  modeHintDismiss?.addEventListener("click", () => {
    try {
      sessionStorage.setItem(MODE_HINT_KEY, "1");
    } catch {
      /* ignore */
    }
  ...
  function renderModeChrome() {
    ...
    if (modeCaption) {
      modeCaption.textContent = simple
        ? "Rooms and devices. Price and cart in one screen."
        : "Per-room detail, panel layout, DIN spec and full BOM.";
    }
    if (modeHint) {
      let dismissed = false;
      try {
        dismissed = sessionStorage.getItem(MODE_HINT_KEY) === "1";
      } catch {
        /* ignore */
      }
      modeHint.hidden = simple || dismissed;
    }
  ...
  function renderSteps() {
    ...
    const tabs = STEPS.map(
      (name, i) =>
        `<button type="button" class="hm-step ${i === state.step ? "is-active" : ""}" data-step="${i}">${i + 1}. ${name}</button>`,
    ).join("");
    const back =
      state.step > 0
        ? `<button type="button" class="hm-step hm-step--back" id="hm-back">Back</button>`
        : "";
    stepsNav.innerHTML = `${tabs}${back}`;
    stepsNav.querySelectorAll(".hm-step[data-step]").forEach((btn) => {
      btn.addEventListener("click", () => {
    ...
    const backBtn = stepsNav.querySelector("#hm-back");
    if (backBtn) {
      backBtn.onclick = () => {
  ...
  function renderSummary(result) {
    ...
    if (tot && tot.pricedQty > 0) {
      priceHtml = `<p class="hm-simple-price">${formatMoney(tot.total)} <span>${escapeAttr(tot.currency)}</span></p>`;
    }
    const ptype =
  ...
  function renderObject() {
    ...
    main.innerHTML = `
      ...
      <details class="hm-sys-section" open>
        ...
        <div class="hm-sys-grid">
          ...
          <div class="hm-room-field">
            <div class="hm-room-field__label"><span>Motion / presence detectors</span></div>
            <select id="f-motion-out">
              <option value="dry_contact" ${(s.motion_output || "dry_contact") !== "transistor" ? "selected" : ""}>Dry contact (relay output)</option>
              <option value="transistor" ${s.motion_output === "transistor" ? "selected" : ""}>3-wire, transistor output</option>
            </select>
          </div>
        </div>
      </details>
    ...
    const motionOut = main.querySelector("#f-motion-out");
    if (motionOut) {
      motionOut.onchange = (e) => {
        state.systems.system.motion_output = e.target.value;
        saveState();
        bump();
      };
    }
  ...
  function panelResultBlock(panel, opts = {}) {
    const { manifest = null, priceMap: prices = priceMap, channelHtml = "" } = opts;
    ...
    for (const m of modules) {
      const p = prices[m.sku];
      const qty = m.qty || 0;
      const din = lineWidthM(m.din_units, qty);
      const priced = p && typeof p.amount === "number";
      if (priced) {
        const line = p.amount * qty;
        modCurrency = p.currency || modCurrency;
        if (p.available === false) unavailSub += line;
        else availSub += line;
      } else {
        missingPrice += 1;
      }
      const availCell =
        priced && p.available === false
          ? pricingBadge("unavailable")
          : priced
            ? pricingBadge("shop")
            : `<span class="hm-muted">—</span>`;
      const row = `<tr data-hl="module:${escapeAttr(m.id)}"${priced && p.available === false ? ' class="hm-bom__oos"' : ""}>
          ...
          <td class="hm-bom__price">${formatPriceCell(p, qty)}</td>
          <td class="hm-bom__avail">${availCell}</td>
        </tr>`;
      if (priced && p.available === false) unavailRows.push(row);
      else availRows.push(row);
    }
    const accessories = panel.accessories || [];
    const knownIds = new Set(OTHER_MATERIAL_CATEGORIES.flatMap((c) => c.ids));
    const catGroups = OTHER_MATERIAL_CATEGORIES.map((cat) => ({
      ...cat,
      items: accessories.filter((a) => cat.ids.includes(a.id)),
    })).filter((g) => g.items.length);
    const orphans = accessories.filter((a) => !knownIds.has(a.id));
    if (orphans.length) {
      catGroups.push({
        id: "other",
        label: "Other panel materials",
    ...
    const isConsumable = (a) => CONSUMABLE_COMPANION_IDS.has(a.id);
    let accLineCount = 0;
    let accUnits = 0;
    let accDin = 0;
    let blankDin = 0;
    ...
    const accRow = (a, { hidden = false } = {}) => {
      const qty = Number(a.qty) || 0;
      accLineCount += 1;
      accUnits += qty;
      if (a.id === "blanking_plate") {
        blankDin += (Number(a.din_units) || 1) * qty;
      } else {
        accDin += (Number(a.din_units) || 0) * qty;
      }
      const line = a?.price?.line_total ?? (a?.price?.amount != null ? a.price.amount * qty : null);
      if (Number.isFinite(line)) {
        accTotal += line;
        accCurrency = a.price.currency || accCurrency || "EUR";
        const cat = OTHER_MATERIAL_CATEGORIES.find((c) => c.ids.includes(a.id));
        if (cat && cat.required === false) optAccTotal += line;
        else reqAccTotal += line;
      }
      if (hidden) hiddenLineCount += 1;
      const din = lineWidthM(a.din_units, qty);
      const note = companionNote(a.id, a.label);
      const lab = a.label || a.id;
      const noteHtml =
        note && note !== lab
          ? `<br><span class="hm-muted">${escapeAttr(note)}</span>`
          : "";
      const kind = companionPricingKind(a);
      const badge = kind ? pricingBadge(kind) : `<span class="hm-muted">—</span>`;
      const cls = hidden ? ' class="hm-acc-consumable" hidden' : "";
      return `<tr data-hl="companion:${escapeAttr(a.id)}"${cls}>
        <td><div class="hm-bom__item">${hmIcon(companionRowIcon(a.id), 18)}<span><strong>${a.qty}×</strong> ${escapeAttr(lab)}${noteHtml}</span></div></td>
        <td class="hm-bom__num">${din}</td>
        <td class="hm-bom__price">${formatCompanionPrice(a)}</td>
        <td class="hm-bom__avail">${badge}</td>
      </tr>`;
    };

    const accBodyParts = [];
    for (const g of catGroups) {
      const primary = g.items.filter((a) => !isConsumable(a));
      const consumable = g.items.filter((a) => isConsumable(a));
      const catAmt = g.items.reduce((s, a) => {
        const line = a?.price?.line_total ?? (a?.price?.amount != null ? a.price.amount * (a.qty || 0) : 0);
        return s + (Number.isFinite(line) ? line : 0);
      }, 0);
      const catCur = g.items.find((a) => a?.price?.currency)?.price?.currency || "EUR";
      const optTag = g.required === false ? " · optional" : "";
      accBodyParts.push(
        `<tr class="hm-bom__cat"><td colspan="4"><strong>${escapeAttr(g.label)}</strong>${optTag}${catAmt > 0 ? ` · ~ ${formatMoney(catAmt)} ${escapeAttr(catCur)}` : ""}</td></tr>`,
      );
      for (const a of primary) accBodyParts.push(accRow(a));
      for (const a of consumable) accBodyParts.push(accRow(a, { hidden: true }));
    }
    ...
    const availLine =
      availSub > 0 ? `${formatMoney(availSub)} ${escapeAttr(modCurrency)}` : "—";
    const unavailLine =
      unavailSub > 0 ? `${formatMoney(unavailSub)} ${escapeAttr(modCurrency)}` : "";
    const under =
      missingPrice > 0
        ? `<tr><td colspan="4"><span class="hm-warn">Available subtotal understated — ${missingPrice} line(s) without a price</span></td></tr>`
        : "";

    const modBody = [
      ...availRows,
      `<tr class="hm-bom__sub"><td>Subtotal (available)</td><td></td><td class="hm-bom__price">${availLine}</td><td></td></tr>`,
      under,
      ...(unavailRows.length
        ? [
            `<tr class="hm-bom__cat"><td colspan="4"><strong>Unavailable today</strong></td></tr>`,
            ...unavailRows,
            `<tr class="hm-bom__sub"><td>Subtotal (unavailable)</td><td></td><td class="hm-bom__price">${unavailLine}</td><td></td></tr>`,
          ]
        : []),
    ].join("");
    ...
    if (reqAccTotal > 0) {
      accSubParts.push(
        `<tr class="hm-bom__sub"><td>Subtotal required</td><td></td><td class="hm-bom__price">~ ${formatMoney(reqAccTotal)} ${escapeAttr(accCurrency)}</td><td></td></tr>`,
      );
    }
    if (optAccTotal > 0) {
      accSubParts.push(
        `<tr class="hm-bom__sub"><td>Subtotal optional</td><td></td><td class="hm-bom__price">~ ${formatMoney(optAccTotal)} ${escapeAttr(accCurrency)}</td><td></td></tr>`,
      );
    }
    if (accTotal > 0) {
      accSubParts.push(
        `<tr class="hm-bom__sub"><td colspan="2">Subtotal (indicative)</td><td class="hm-bom__price">~ ${formatMoney(accTotal)} ${escapeAttr(accCurrency)}</td><td></td></tr>`,
      );
    }

    const moreBtn =
      hiddenLineCount > 0
        ? `<tr><td colspan="4"><button type="button" class="hm-linkish" data-acc-more data-n="${hiddenLineCount}">Show ${hiddenLineCount} more line${hiddenLineCount === 1 ? "" : "s"}</button></td></tr>`
        : "";
    ...
    const yamlBlock = yaml
      ? `<details class="hm-fold">
          <summary>${hmIcon("code", 18)} ESPHome — RS-485 bus timings <span class="hm-fold__chev">${hmIcon("chevron", 16)}</span></summary>
          <div class="hm-fold__body">
            <button type="button" class="hm-btn-pill hm-btn-pill--ghost hm-yaml-copy" data-yaml-copy>Copy</button>
            <pre class="hm-yaml-pre">${yaml}</pre>
          </div>
        </details>`
      : "";

    const encCompanions = panel.enclosure?.din_companions;
    const accDinLabel =
      encCompanions != null
        ? `${Math.round(Number(encCompanions) * 100) / 100} M of DIN width${
            blankDin > 0 ? ` (+ ${Math.round(blankDin * 100) / 100} M blanking)` : ""
          }`
        : `${Math.round(accDin * 100) / 100} M of DIN width${
            blankDin > 0 ? ` (+ ${Math.round(blankDin * 100) / 100} M blanking)` : ""
          }`;
    ...
    return `
      <section class="hm-panel-result">
        ...
        <div class="hm-card">
          ...
          <p class="hm-bom__legend">${pricingBadge("shop")} shop price · ${pricingBadge("unavailable")} cannot order today</p>
          <table class="hm-bom">
            <thead><tr><th>Item</th><th class="hm-bom__num">Line width</th><th class="hm-bom__price">Price</th><th>Availability</th></tr></thead>
            <tbody>${modBody || `<tr><td colspan="4" class="hm-muted">—</td></tr>`}</tbody>
          </table>
        </div>
        <div class="hm-card hm-card--paper" id="hm-companions">
          <div class="hm-acc-head">
            <h4>Companions &amp; materials</h4>
            <span class="hm-muted">${accLineCount} line item${accLineCount === 1 ? "" : "s"} · ${accUnits} unit${accUnits === 1 ? "" : "s"} · ${accDinLabel}</span>
          </div>
          <p class="hm-bom__legend">${pricingBadge("indicative")} confirmed indicative · ${pricingBadge("estimate")} estimate · not sold by HomeMaster · not added to cart</p>
          <table class="hm-bom hm-bom--acc">
            <thead><tr><th>Item</th><th class="hm-bom__num">Line width</th><th class="hm-bom__price">Price</th><th>Pricing</th></tr></thead>
            <tbody>
              ${accBodyParts.join("")}
              ${moreBtn}
              ${accSubParts.join("")}
  ...
  async function renderResult() {
    ...
    if (!estimateHasUserDemand(result, rules)) {
      main.innerHTML = `
        <div class="hm-result">
          <div class="hm-card hm-invite">
            <h3 class="hm-invite__h">Describe your home to get an estimate</h3>
            <p class="hm-invite__lead">Add rooms and equipment on the previous steps — lights, switches, heating, leak protection and more. The panel estimate appears here once there is something to size.</p>
            <p class="hm-muted">Network and server lines are not shown until you add equipment.</p>
          </div>
        </div>`;
      return;
    }
    ...
    const comm = buildCommercialSummary(result, priceMap, {
      rooms: state.rooms,
    ...
    const totalsParts = equipmentTotalsHtml({
      cartTot: tot,
      sysTot,
      panelTot,
      comm,
    ...
    const channelHtml = channelUsageTable(result.channel_usage, { open: true });
    const panelBlocks = (result.panels || [])
      .map((p, i) =>
        panelResultBlock(p, {
          manifest,
          priceMap,
          channelHtml: i === 0 ? channelHtml : "",
        }),
      )
      .join("");

    const dinLabel =
      dinW === "—" || dinW == null ? "—" : `${dinW} M`;
    const hero = resultHeroHtml({
      priceHtml: totalsParts.body,
      legalHtml: totalsParts.legal,
      stats: [
        { icon: "cube", value: modQty, label: pluralWord(modQty, "Module", "Modules") },
        { icon: "panel", value: panelN, label: pluralWord(panelN, "Panel", "Panels") },
        { icon: "ruler", value: dinLabel, label: "DIN width" },
        { icon: "layers", value: sections, label: pluralWord(sections, "Section", "Sections") },
      ],
      actionsHtml: `
            <button type="button" class="hm-btn-pill hm-btn-pill--primary" id="hm-cart">${cartAddButtonLabel(tot)}</button>
            <p class="hm-result-hero__cart-hint">${escapeAttr(
              tot && tot.pricedQty > 0
                ? homemasterStockLabel(comm.homemasterCartTotal) ||
                    `${tot.lineCount} line(s) · ${tot.pricedQty} unit(s) available now`
                : "No HomeMaster modules available to add today",
            )}</p>
            <div class="hm-result-dl">
              <button type="button" class="hm-btn-pill hm-btn-pill--ghost" id="hm-xlsx">${hmIcon("download", 16)} Download panel BOM (.xlsx)</button>
              <p class="hm-result-xlsx__note">HomeMaster modules, enclosure, PSU, protection and terminals. No wiring diagrams, cable schedules or installer quote.</p>
            </div>
    ...
    main.querySelectorAll("[data-acc-more]").forEach((btn) => {
      btn.onclick = () => {
        ...
        btn.textContent = show ? `Hide extras` : `Show ${n} more line${n === "1" ? "" : "s"}`;
      };
    });
    main.querySelectorAll("[data-yaml-copy]").forEach((btn) => {
      btn.onclick = async () => {
  ...
  function countRow(id, label, value, min, max, icon) {
    return `<div class="hm-count-row" data-slider="${escapeAttr(id)}">
      <span class="hm-count-row__lab">${icon ? hmIcon(icon, 16) : ""}<span>${escapeAttr(label)}</span></span>
      <span class="hm-count-row__ctrl">
        <button type="button" class="hm-count-row__btn" data-for="${escapeAttr(id)}" data-step="-1" aria-label="Decrease">−</button>
        <input type="number" class="hm-count-row__input" id="${escapeAttr(id)}" min="${min}" max="${max}" value="${value}" inputmode="numeric" aria-label="${escapeAttr(label)}">
        <button type="button" class="hm-count-row__btn" data-for="${escapeAttr(id)}" data-step="1" aria-label="Increase">+</button>
      </span>
    </div>`;
  }
  ...
  function renderSimple() {
    ...
    const roomSliders = SIMPLE_ROOM_TYPES.map(({ id, label }) =>
      countRow(`sc-${id}`, label, c[id] ?? 0, 0, 10, roomIcons[id]),
    ).join("");
    ...
    const equipIsCore = (id) => {
      if (id === "lights_dimmable") return lightsOn || (Number(t.lights_dimmable) || 0) > 0;
      return SIMPLE_EQUIP_CORE.has(id);
    };
    const equipVisible = (id) =>
      equipIsCore(id) || (Number(t[id]) || 0) > 0;
    ...
    const equipActiveHtml = activeEquip
      .map(([id, label, max, icon]) => countRow(`st-${id}`, label, t[id] ?? 0, 0, max, icon))
      .join("");
    ...
    const equipAdd =
      hiddenEquip.length > 0
        ? `<details class="hm-add-equip"><summary><span class="hm-details-chev" aria-hidden="true">${hmIcon("chevron", 14)}</span> Add equipment</summary><div class="hm-slider-stack">${equipHiddenHtml}</div></details>`
        : "";
      ...
      <div class="hm-simple">
        <div class="hm-simple__settings">
          ...
          <section class="hm-simple-sec">
            ...
            ${equipAdd}
            <label class="hm-simple-motion-out">Motion / presence detectors — output
              <select id="s-motion-out">
                <option value="dry_contact" ${(state.systems?.system?.motion_output || "dry_contact") !== "transistor" ? "selected" : ""}>Dry contact (relay output)</option>
                <option value="transistor" ${state.systems?.system?.motion_output === "transistor" ? "selected" : ""}>3-wire, transistor output</option>
              </select>
            </label>
          </section>
    ...
    const setValDisplays = (id, n) => {
      const input = main.querySelector(`#${CSS.escape ? CSS.escape(id) : id}`);
    ...
    const sMotionOut = main.querySelector("#s-motion-out");
    if (sMotionOut) {
      sMotionOut.onchange = (e) => {
        if (!state.systems) state.systems = emptySystems(firstPanelId());
        if (!state.systems.system) state.systems.system = {};
        state.systems.system.motion_output = e.target.value;
        runSimpleSync();
        saveState();
        paintSimpleResult();
  ...
  async function paintSimpleResult() {
    ...
    if (!estimateHasUserDemand(result, rules)) {
      host.innerHTML = `
        <div class="hm-card hm-invite">
          <h3 class="hm-invite__h">Describe your home to get an estimate</h3>
          <p class="hm-invite__lead">Set room counts and equipment above — lights, switches, heating, leak protection and more. The estimate appears here once there is something to size.</p>
          <p class="hm-muted">Network and server lines are not shown until you add equipment.</p>
        </div>`;
      return;
    }
    const directory = await ensurePartnersDirectory();
    const hm = comm.homemasterCartTotal;
    const rows = comm.moduleRows;
    ...
    const stock = homemasterStockLabel(hm);
    const buyLabel =
      hm && hm.pricedQty > 0
        ? `${hmIcon("cart", 16)} Buy HomeMaster modules — ${formatMoney(hm.total)} ${escapeAttr(hm.currency)}`
        : `${hmIcon("cart", 16)} Buy HomeMaster modules`;

    host.innerHTML = `
      ${shareOlder}
      ${notesBar}
      <div class="hm-simple-result__stack">
        <div class="hm-card hm-card--commerce">
          ${simpleHomemasterPriceHtml(comm)}
          <div class="hm-simple-buy hm-simple-buy--nested">
            <h3 class="hm-simple-buy__h">This configuration covers</h3>
            <ul class="hm-simple-lines">${coverList || "<li class='hm-muted'>Adjust the controls to build your system.</li>"}</ul>
            <h3 class="hm-simple-buy__h">What you buy from HomeMaster</h3>
            <p class="hm-simple-buy__note">These lines go to the cart. Other panel materials are sourced separately.</p>
            ${modulesYouBuyHtml(rows)}
            <div class="hm-simple-cta">
              <button type="button" class="hm-btn-pill hm-btn-pill--primary hm-simple-cta__btn" id="s-cart">${buyLabel}</button>
              <p class="hm-simple-cta__status">${escapeAttr(stock)}</p>
            </div>
          </div>
        </div>
        ${simpleInstallerHtml(directory, installerCountryId)}
        ${simpleOtherMaterialsHtml(comm)}
        ${simpleCompleteHardwareHtml(comm)}
        ${simpleInstallerQuotesHtml(comm)}
        <div class="hm-simple-actions-secondary">
          <button type="button" class="hm-btn-pill hm-btn-pill--ghost hm-simple-xlsx" id="s-xlsx">${hmIcon("download", 16)} Download panel BOM (.xlsx)</button>
          <p class="hm-simple-xlsx__note">HomeMaster modules, enclosure, PSU, protection and terminals. No wiring diagrams, cable schedules or installer quote.</p>
          <div class="hm-simple-cta__links">
            <button type="button" class="hm-text-link" id="s-fullspec">${hmIcon("gear", 16)} Configure in detail</button>
            <button type="button" class="hm-text-link" id="s-share">${hmIcon("link", 16)} Copy link</button>
          </div>
        </div>
      </div>
      ${warn}
    ...
    const expandMats = host.querySelector("#s-mat-expand");
    if (expandMats) {
      expandMats.onclick = () => {
        host.querySelectorAll(".hm-mat__cat").forEach((d) => {
          d.open = true;
        });
        const wrap = host.querySelector(".hm-mat");
        if (wrap) wrap.open = true;
      };
    }
    host.querySelector("#s-fullspec").onclick = () =>
      setUiMode("advanced", { step: 3, scrollTo: "#hm-companions" });
    ...
    if (simpleDock) {
      const dockPrice =
        hm && hm.pricedQty > 0
          ? `${formatMoney(hm.total)} ${escapeAttr(hm.currency)}`
          : "—";
      simpleDock.hidden = false;
      simpleDock.innerHTML = `
        <button type="button" class="hm-btn-pill hm-btn-pill--primary hm-simple-dock__cart" id="s-dock-cart">${hmIcon("cart", 16)} Buy HomeMaster modules — ${dockPrice}</button>
        <p class="hm-simple-dock__hint">Complete panel also needs separately sourced materials.</p>`;
