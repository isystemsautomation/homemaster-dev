 ...
 * Used by Advanced Result. Simple mode uses simpleEquipmentCardHtml.
...
/**
 * Simple mode: one Equipment total (when currencies match) + breakdown + installation line.
 * @param {{ sysTot: object|null, panelTot: object|null, meta?: object|null }} opts
 */
function simpleEquipmentCardHtml({ sysTot, panelTot, meta = null }) {
  const hmOk = sysTot && sysTot.pricedQty > 0 && Number.isFinite(sysTot.total);
  const panelOk = panelTot && panelTot.lineCount > 0 && Number.isFinite(panelTot.total);
  const hmCur = hmOk ? sysTot.currency || "EUR" : null;
  const panelCur = panelOk ? panelTot.currency || "EUR" : null;
  const sameCur = hmOk && panelOk && hmCur === panelCur;

  let heroAmount = "—";
  let heroLabel = "Equipment total";
  if (sameCur) {
    const sum = Math.round((sysTot.total + panelTot.total) * 100) / 100;
    heroAmount = `~ ${formatMoney(sum)} <span>${escapeAttr(hmCur)}</span>`;
  } else if (hmOk && panelOk) {
    heroAmount = `${formatMoney(sysTot.total)} <span>${escapeAttr(hmCur)}</span> + ~ ${formatMoney(panelTot.total)} <span>${escapeAttr(panelCur)}</span>`;
  } else if (hmOk) {
    heroAmount = `${formatMoney(sysTot.total)} <span>${escapeAttr(hmCur)}</span>`;
    heroLabel = "HomeMaster modules";
  } else if (panelOk) {
    heroAmount = `~ ${formatMoney(panelTot.total)} <span>${escapeAttr(panelCur)}</span>`;
  }

  const hmLine = hmOk
    ? `<div class="hm-equip__line">
        <span class="hm-equip__name">HomeMaster modules</span>
        <span class="hm-equip__amt">${formatMoney(sysTot.total)} ${escapeAttr(hmCur)}</span>
        <span class="hm-equip__tag">exact, from the shop</span>
      </div>`
    : `<div class="hm-equip__line">
        <span class="hm-equip__name">HomeMaster modules</span>
        <span class="hm-equip__amt">—</span>
        <span class="hm-equip__tag">exact, from the shop</span>
      </div>`;

  const panelLine = panelOk
    ? `<div class="hm-equip__line">
        <span class="hm-equip__name">Panel equipment</span>
        <span class="hm-equip__amt">~ ${formatMoney(panelTot.total)} ${escapeAttr(panelCur)}</span>
        <span class="hm-equip__tag">indicative retail</span>
      </div>`
    : `<div class="hm-equip__line">
        <span class="hm-equip__name">Panel equipment</span>
        <span class="hm-equip__amt">—</span>
  ...
  const tip =
    panelTot?.disclaimer ||
    meta?.disclaimer_en ||
    `Indicative retail prices, EU, as of ${asOf}. Installation typically adds 40–80 % depending on country and new-build vs retrofit.`;

  return `<div class="hm-card hm-equip">
    <p class="hm-equip__hero-amt">${heroAmount}</p>
    <p class="hm-equip__hero-lab">${escapeAttr(heroLabel)}</p>
    <div class="hm-equip__break">
      ${hmLine}
      ${panelLine}
    </div>
    <div class="hm-equip__install-row">
      <span class="hm-equip__name">Installation</span>
      <span class="hm-equip__install-note">not included — quoted by your installer</span>
    </div>
    <p class="hm-equip__tip" title="${escapeAttr(tip)}">
      <span class="hm-equip__tip-ico" aria-hidden="true">i</span>
      Indicative panel prices · installation typically +40–80%
    </p>
  </div>`;
}
...
function mountConfigurator(root) {
  ...
  async function paintSimpleResult() {
    ...
    const coverList = lines
      ...
      .join("");
    const priceHtml = simpleEquipmentCardHtml({
      sysTot,
      panelTot,
      meta: result.reference_prices_meta,
    });
    ...
    const cartBtnLabel =
      cartTot && cartTot.pricedQty > 0
        ? `${hmIcon("cart", 16)} Add to cart · ${formatMoney(cartTot.total)} ${escapeAttr(cartTot.currency)}`
        : `${hmIcon("cart", 16)} Add to cart`;
    ...
    host.innerHTML = `
      ...
      <div class="hm-card hm-simple-buy">
        ...
        <div class="hm-simple-cta">
          <button type="button" class="hm-btn-pill hm-btn-pill--primary hm-simple-cta__btn" id="s-cart">${cartBtnLabel}</button>
          ${simpleCartStatusHtml(cartTot, sysTot)}
          <div class="hm-simple-cta__links">
            <button type="button" class="hm-text-link" id="s-xlsx">${hmIcon("download", 16)} Download</button>
    ...
    if (simpleDock) {
      const dockPrice =
        cartTot && cartTot.pricedQty > 0
          ? `${formatMoney(cartTot.total)} ${escapeAttr(cartTot.currency)}`
          : "—";
