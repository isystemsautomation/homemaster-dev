...
// IREG 21..25: PWM 12-bit instantaneous level, mid-fade (diagnostic only)
// IREG 26..28: STATE readback — TARGET levels (pwmTarget, API 0..255) + flags
//   26: (R<<8)|G   27: (B<<8)|WW   28: (CW<<8)|flags
//   flags low byte: bit0 anyOn, bit1 rgbGroupOn, bit2 cctGroupOn,
//                   bit3 relay1, bit4 fading (any channel still slewing)
//   v0.3.0: 26..28 report the target, not the in-flight value. A master that
//   restores entity state from this block must not sample mid-transition.
enum : uint16_t {
  IREG_DI_MASK      = 0,
  IREG_RLY_MASK     = 1,
  IREG_BTN_MASK     = 2,
...
void updateInputRegisters(uint32_t now) {
  ...
  // IREG 21-25 — диагностика: мгновенный уровень в середине перехода.
  for (int i = 0; i < NUM_PWM; i++) {
    mb.setIreg(IREG_PWM_RAW_BASE + i, pwmCurrent[i]);
  }
  // IREG 26-28 — авторитетное состояние: ЦЕЛЬ, а не точка перехода.
  // Мастер (ESPHome) восстанавливает из этого блока состояние сущности light.
  // Публикация pwmCurrent приводила к тому, что опрос, попавший в середину
  // fadeMs, фиксировал промежуточное значение как конечное.
  const uint8_t rApi  = pwmHiToApi(pwmTarget[0]);
  const uint8_t gApi  = pwmHiToApi(pwmTarget[1]);
  const uint8_t bApi  = pwmHiToApi(pwmTarget[2]);
  const uint8_t wwApi = pwmHiToApi(pwmTarget[3]);
  const uint8_t cwApi = pwmHiToApi(pwmTarget[4]);
  mb.setIreg(IREG_STATE_BASE + 0, (uint16_t)((rApi << 8) | gApi));
  mb.setIreg(IREG_STATE_BASE + 1, (uint16_t)((bApi << 8) | wwApi));

  uint8_t stFlags = 0;
  for (int i = 0; i < NUM_PWM; i++) {
    if (pwmTarget[i] > 0) { stFlags |= 0x01; break; }
  }
  if (pwmTarget[0] > 0 || pwmTarget[1] > 0 || pwmTarget[2] > 0) stFlags |= 0x02;
  if (pwmTarget[3] > 0 || pwmTarget[4] > 0) stFlags |= 0x04;
  if (mb.Ists(ISTS_RLY_BASE)) stFlags |= 0x08;
  // bit4 FADING: хотя бы один канал ещё едет к цели. Мастер может
  // пропустить обновление состояния, пока бит взведён.
  for (int i = 0; i < NUM_PWM; i++) {
    if (pwmCurrent[i] != pwmTarget[i]) { stFlags |= 0x10; break; }
  }
...
void loop() {
  ...
  for (int i=0;i<NUM_PWM;i++) {
    ...
    if (apiChg || hiChg) {
      ...
      prevPwm[i]   = (uint16_t)mb.Hreg(HR_PWM_BASE + i);
      prevPwmHi[i] = (uint16_t)mb.Hreg(HR_PWM_HI_BASE + i);
      pwmChanged = true;
    }
  }
  if (pwmChanged) {
